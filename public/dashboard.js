...
function renderGuildCards(guilds = state.guilds) {
    ...
    $$('.choose-server').forEach((card) => {
        ...
        card.addEventListener('keydown', (event) => { if (event.key === 'Enter' || event.key === ' ') {
            ...
            selectGuild(card.dataset.id);
        } });
    });
    $('#noServers').classList.toggle('hidden', visible.length > 0 || guilds.length > 0);
    if (guilds.length > 0 && visible.length === 0)
        grid.innerHTML = '<div class="inline-empty"><i class="fa-solid fa-magnifying-glass"></i><span>Không tìm thấy server phù hợp.</span></div>';
}
...
async function loadLiveRooms(force = false) {
    if (!state.guildId || state.liveRoomsLoading)
        return;
    ...
    if (!state.liveRooms.length)
        grid.innerHTML = '<div class="member-empty"><i class="fa-solid fa-circle-notch fa-spin"></i><strong>Đang đồng bộ voice</strong><span>Đang đọc phòng và người đang kết nối.</span></div>';
    try {
        ...
        if (!state.liveRooms.length)
            grid.innerHTML = `<div class="member-empty"><i class="fa-solid fa-triangle-exclamation"></i><strong>Không tải được phòng</strong><span>${esc(error.message)}</span></div>`;
        else
            toast(error.message, 'bad');
    }
    finally {
...
async function runLiveRoomAction(channelId, action) {
    ...
    else if (action === 'delete' && !window.confirm(`Xóa phòng “${room.name}”?`))
        return;
    try {
...
async function saveRoomPassword(event) {
    ...
    if (!channelId)
        return closeRoomPasswordDialog();
    try {
...
async function loadMembers(force = false) {
    if (!state.detail?.canManage || state.membersLoading)
        return;
    if (!force && state.membersLoadedAt && Date.now() - state.membersLoadedAt < 5 * 60_000) {
        renderMembers();
        return;
    }
    if (!force && state.membersRetryAt > Date.now())
        return;
    state.membersLoading = true;
    const table = $('#membersTable');
    if (!state.members.length)
        table.innerHTML = '<div class="member-empty"><i class="fa-solid fa-circle-notch fa-spin"></i><strong>Đang fetch thành viên</strong><span>Discord chỉ được gọi lại khi cache hết hạn hoặc admin bấm Fetch lại.</span></div>';
    try {
...
async function deleteProduct(id) {
    if (!window.confirm('Xóa gói này?'))
        return;
    try {
        await api(`/api/guilds/${state.guildId}/products/${id}`, { method: 'DELETE' });
        state.detail.products = state.detail.products.filter((item) => item.id !== id);
        renderProducts(state.detail.products);
        toast('Đã xóa gói.');
    }
    catch (error) {
...
async function loadPayments(quiet = false) {
    try {
        ...
        if (!quiet)
            toast('Đã làm mới payment ledger.');
    }
    catch (error) {
        if (!quiet)
            toast(error.message, 'bad');
...
async function loadSepayAccounts(force = false) {
    if (!state.isDeveloper || !state.guildId || state.sepayAccountsLoading)
        return;
    if (!force && state.sepayAccounts.length && Date.now() - state.sepayAccountsLoadedAt < 10 * 60_000)
        return renderSepayAccounts();
    ...
    try {
        ...
        state.sepayAccountsLoading = false;
    }
}
function renderSepayAccounts(selectedId = state.detail?.settings?.sepayBankAccountId || '') {
    ...
    if (selectedId && state.sepayAccounts.some((account) => account.id === selectedId))
        select.value = selectedId;
    else if (state.sepayAccounts[0])
        select.value = state.sepayAccounts[0].id;
}
...
async function postPanel() {
    ...
    try {
        const result = await api(`/api/guilds/${encodeURIComponent(state.guildId)}/payment-panel`, { method: 'POST' });
...
async function init() {
    ...
    $('#inviteBotButton').addEventListener('click', () => { if (state.runtime?.inviteUrl)
        window.open(state.runtime.inviteUrl, '_blank', 'noopener');
    else
        toast('Chưa có Client ID để tạo invite link.', 'bad'); });
    ...
    try {
        ...
        if (requestedPage && pageNames[requestedPage] && state.guildId)
