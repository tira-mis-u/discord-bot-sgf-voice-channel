// Frontend source is TypeScript. Browser output is generated to public/checkout.js.
// @ts-nocheck
...
function setupCopyButtons() { document.querySelectorAll('.copy-payment-value').forEach((button) => button.addEventListener('click', async () => { const target = document.getElementById(button.dataset.copyTarget); const value = target?.textContent?.replace(/[^0-9]/g, '') && button.dataset.copyTarget === 'paymentAmount' ? target.textContent.replace(/[^0-9]/g, '') : target?.textContent || ''; await navigator.clipboard?.writeText(value); button.classList.add('copied'); window.setTimeout(() => button.classList.remove('copied'), 900); })); }
async function init() { setupCopyButtons(); try {
    const session = await api('/api/session');
    if (!session.authenticated) {
        showLogin();
        return;
    }
    const data = await api(`/api/public/payments/${encodeURIComponent(id)}`);
    render(data);
    if (data.payment.status === 'pending')
        setInterval(async () => { try {
            const latest = await api(`/api/public/payments/${encodeURIComponent(id)}`);
            render(latest);
        }
        catch { } }, 5000);
}
