...
function setPage(page) {
  ...
  if (!allowed.includes(page)) page = admin ? 'voice' : 'store';
...
function renderDetail() {
  ...
  if (state.detail.canManageMoney) loadPayments(true);
}

async function loadDeveloperSystem(force = false) {
  if (!state.isDeveloper || state.developerSystemLoading) return;
  ...
  finally { state.developerSystemLoading = false; }
}
...
async function manageMemberPremium(userId, action) {
  if (!state.isDeveloper || !state.guildId) return;
  let days = 30;
  if (action === 'revoke') {
    if (!window.confirm('Thu hồi toàn bộ entitlement Premium đang active của member này?')) return;
    days = 0;
  } else {
    ...
    if (!Number.isFinite(days)) return toast('Số ngày không hợp lệ.', 'bad');
  }
  try {
